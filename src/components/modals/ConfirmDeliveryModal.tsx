...
import { X, User, CheckCircle, Calendar, Truck } from 'lucide-react';

interface ConfirmDeliveryModalProps {
    isOpen: boolean;
    onClose: () => void;
    onConfirm: (data: { deliveryPerson: string; receivedBy: string; deliveryDate: string }) => void;
...
const ConfirmDeliveryModal: React.FC<ConfirmDeliveryModalProps> = ({ isOpen, onClose, onConfirm, removal }) => {
    ...
    useEffect(() => {
        if (removal) {
            setDeliveryPerson(removal.deliveryPerson || '');
        }
    }, [removal]);
    ...
    const handleConfirmClick = () => {
        if (deliveryPerson.trim() && receivedBy.trim() && deliveryDate) {
            onConfirm({ deliveryPerson, receivedBy, deliveryDate });
            onClose();
        } else {
            ...
            <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
                ...
                <div className="bg-gray-50 p-4 border-t flex justify-end items-center gap-4">
                    ...
                    <button
                        ...
                        disabled={!deliveryPerson.trim() || !receivedBy.trim() || !deliveryDate}
                    >
                        Confirmar
                    </button>
                </div>
